...
(function(__global__){
    var
        attrs= {
            ...
            // be careful when using __call__
            // the function provided will be called with the first two parameters
            // being the current context object and the second argument being
    ...
    Class.create= function(kwargs) {
        ...
        __class__= function(){
            ...
            if( !(this instanceof __class__) || !instanceInitializing ) {
                ...
                callableObject= function(){
                    return newInstance.__call__.apply(newInstance, [this, callableObject].concat(Array.prototype.slice.call(arguments)));
                };
                
                for(name in newInstance) {
                    callableObject[name] = newInstance[name];
                }
        ...
        for(name in kwargs) {
            if(kwargs.hasOwnProperty(name)) {
                ...
                prototype[name]= (function(proto, name, kwargs){
                    
                    // Check if we're overwriting an existing function
                    if(typeof kwargs[name] == "function" && typeof _super[name] == "function" && fnTest.test(kwargs[name])) {
                        return function() {
                            var
                                tmp = this._super
                                ,ret
                            ...
                            ret= kwargs[name].apply(this, arguments);
                            this._super= tmp;
                        ...
                        // copy the attribute to the prototype
                        return kwargs[name];
                        
                    }
                    
                })(prototype, name, kwargs);
        ...
        if(kwargs && kwargs.__name__) {
            var
                current= kwargs.__parent__ || __global__
            ...
            __class__.__fullname__= kwargs.__name__;
        }
        /*
        if(kwargs.__name__) {
            __class__.__name__= kwargs.__name__.split('.').slice(-1)[0];
        }
        */
        ...
        if(kwargs && kwargs.__metaclass__) {
            metaClass= kwargs.__metaclass__(__class__);
        } else if(_super.__metaclass__) {
            metaClass= _super.__metaclass__(__class__);
        }
